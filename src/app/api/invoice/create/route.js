...
export async function GET(req) {
    try {
        ...
        const productCards = await prisma.productCards.findMany({
            where: {userId: id},
        ...
        productCards.forEach((element) => {
            let price;
            if(element['product']['discount_price']){
                price = element['product']['discount_price']
            }else{
        ...
        let vat = totalAmount * 0.05;  // Assuming VAT is 5% of the total amount
        let payable = totalAmount + vat;

        // Step 02: Prepare Customer Details & Shipping Details
        let Profile = await prisma.customerProfiles.findUnique({where: {userId: id}});
        ...
        for(const element of productCards){
            await prisma.invoiceProducts.create({
                data: {
                    ...
                    userId: id,
                    qty: element.qty,
                    sale_price: element.product.discount_price ? element.product.discount_price : element.product.price,
                    color: element.color,
                    size: element.size
                }
            })
        ...
        await prisma.productCards.deleteMany({where: {userId: id}});
